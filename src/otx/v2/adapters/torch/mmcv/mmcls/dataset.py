...
from mmcls.datasets import (
    build_dataset as mmcls_build_dataset,
)
from mmcls.datasets import (
    build_sampler,
)
from mmcls.datasets.builder import worker_init_fn
from otx.v2.adapters.torch.mmcv.mmcls.modules.datasets import (
    OTXClsDataset,
    OTXHierarchicalClsDataset,
    OTXMultilabelClsDataset,
    SelfSLDataset,
)
from otx.v2.adapters.torch.mmcv.modules.utils import MPAConfig
...
class Dataset(BaseDataset):
    ...
    def build_dataset(
        ...
        return mmcls_build_dataset(config)
            ...
            if distributed:
                sampler = build_sampler(
                    dict(
                        ...
                        rank=rank,
                        shuffle=shuffle,
                        round_up=kwargs.get("round_up", True),
                        seed=seed,
                    )
                )
            else:
                # shuffle=False when val and test
                sampler.update(shuffle=shuffle)
                sampler = build_sampler(
                    sampler, default_args=dict(dataset=dataset, num_replicas=world_size, rank=rank, seed=seed)
                )
