...
class CythonAugments(Augments):
    ...
    def autocontrast(img: ImgTypes, *args, **kwargs) -> ImgTypes:
        """Apply autocontrast for an given image."""
        if CYTHON_MODULE_ENABLED and Image.isImageType(img):
    ...
    def blend(src: ImgTypes, dst: ndarray, weight: float = 0.0) -> ImgTypes:
        ...
        if CYTHON_MODULE_ENABLED and Image.isImageType(src):
