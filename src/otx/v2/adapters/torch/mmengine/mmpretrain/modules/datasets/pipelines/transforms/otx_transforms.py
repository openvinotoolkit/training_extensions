...
class TensorNormalize:
    ...
    def __init__(self, mean: list, std: list, inplace: bool = False) -> None:
        self.mean = mean
        self.std = std
...
class RandomRotate:
    ...
    def __init__(self, p: float = 0.5, angle: tuple = (-5, 5), values: Optional[list] = None) -> None:
        self.p = p
        self.angle = angle
        self.discrete = values is not None and len([v for v in values if v != 0]) > 0
        self.values = values

    def __call__(self, results: dict, *args, **kwargs) -> dict:
        ...
        for key in results.get("img_fields", ["img"]):
            img = results[key]
            if self.discrete and self.values is not None:
                # disable B311 random - used for the random sampling not for security/crypto
...
class PackMultiKeyInputs(PackInputs):
    def __init__(self, multi_key: list = [], **kwargs) -> None:
