...
class CustomRoIHead(StandardRoIHead):
    ...
    def init_bbox_head(self, bbox_roi_extractor: Config | ConfigDict, bbox_head: Config | ConfigDict) -> None:
        """Initialize ``bbox_head``.

        Args:
            bbox_roi_extractor(Config | ConfigDict): A config for building bbox_roi_extractor.
            bbox_head(Config | ConfigDict): A config for building bbox_head.
        """
    ...
    def loss(self, x: tuple[Tensor], rpn_results_list: InstanceList, batch_data_samples: list[DetDataSample]) -> dict:
        ...
        if len(rpn_results_list) != len(batch_data_samples):
            msg = "Length of rpn_results_list and batch_data_samples are different"
...
class CustomConvFCBBoxHead(Shared2FCBBoxHead, CrossDatasetDetectorHead):
    ...
    def loss(
        ...
        bbox_targets: Tensor,
        bbox_weights: Tensor,
        valid_label_mask: list[Tensor] | None = None,
        reduction_override: str | None = None,
    ) -> dict:
