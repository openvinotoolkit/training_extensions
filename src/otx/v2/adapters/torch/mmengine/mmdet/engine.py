...
class MMDetEngine(MMXEngine):
    ...
    def predict(
        ...
        **kwargs,
        ...
        Args:
            model (Optional[Union[Dict, str]], optional): The model to use for inference.
                Can be a dictionary containing the model configuration, or a string representing the path to
        ...
        # Update pipelines
        if pipeline is None:
            from otx.v2.adapters.torch.mmengine.mmdet.dataset import get_default_pipeline

            pipeline = get_default_pipeline()
        config = Config({})
        if isinstance(model, dict) and "_config" in model:
        ...
        if task is not None and task != "Object Detection":
