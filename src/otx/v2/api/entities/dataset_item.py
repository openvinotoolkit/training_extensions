...
from typing import Any, List, Optional, Sequence, Set, Tuple, TypeVar, Union

import numpy as np
from bson import ObjectId

from otx.v2.api.entities.annotation import Annotation, AnnotationSceneEntity
...
from otx.v2.api.entities.metadata import IMetadata, MetadataItemEntity
...
class DatasetItemEntity(metaclass=abc.ABCMeta):
    ...
    def append_metadata_item(self, data: IMetadata, model: Any = None):
        ...
        >>> from otx.v2.api.entities.tensor import TensorEntity
    ...
    def get_metadata_by_name_and_model(self, name: str, model: Any) -> Sequence[MetadataItemEntity]:
        """Returns a metadata item with `name` and generated by `model`.

        Args:
            name (str): the name of the metadata
            model (Any): the model which was used to generate the metadata.
