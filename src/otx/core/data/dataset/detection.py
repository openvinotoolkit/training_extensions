...
class OTXDetectionDataset(OTXDataset[DetDataEntity]):
    ...
    def _get_item_impl(self, index: int) -> DetDataEntity | None:
        ...
        entity = DetDataEntity(
            ...
            else torch.as_tensor(bboxes),  # to avoid resizing bounding boxes when validation and testing
            labels=torch.as_tensor([ann.label for ann in bbox_anns]),
        )

        transformed_entity = self._apply_transforms(entity)
        if self.dm_subset.name != "train":
            transformed_entity.bboxes = tv_tensors.BoundingBoxes(  # type: ignore[union-attr]
                entity.bboxes,
                format=tv_tensors.BoundingBoxFormat.XYXY,
