...
from typing import Tuple, Type, Sequence, TypeVar
...
def draw_bboxes(img, bboxes):
    ...
    img = img.copy()
    for bbox in bboxes:
        left_top = bbox[:2].astype(np.int32)
        right_bottom = np.round(bbox[2:]).astype(np.int32)
        # draw bounding box with openCV
        img = cv2.rectangle(img, tuple(left_top), tuple(right_bottom), (0, 0, 255))
    return img


def rescale(boxes, scale_factor: Tuple[float, float]) -> None:
...
def translate(boxes, distances: Tuple[float, float]) -> None:
...
def flip(boxes, img_shape: Tuple[int, int], direction: str = 'horizontal') -> None:
...
def clip(boxes, img_shape: Tuple[int, int]) -> None:
...
def polygon__get_item(self, index):
...
class CachedMosaic(Mosaic):
    ...
    def __call__(self, results: dict) -> dict:
        ...
        results['gt_bboxes'] = mosaic_bboxes.astype(np.float32)
...
class CachedMixUp:
    ...
    def __call__(self, results: dict) -> dict:
        ...
        mixup_gt_bboxes_labels = np.concatenate(
            (results['gt_labels'], retrieve_gt_bboxes_labels), axis=0)
