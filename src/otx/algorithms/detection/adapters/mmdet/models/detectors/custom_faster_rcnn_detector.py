...
class CustomFasterRCNN(SAMDetectorMixin, L2SPDetectorMixin, FasterRCNN):
    """CustomFasterRCNN Class for mmdetection detectors."""
    ...
    def load_state_dict_pre_hook(model, model_classes, chkpt_classes, chkpt_dict, prefix, *args, **kwargs):
        ...
        logger.info(f"----------------- CustomFasterRCNN.load_state_dict_pre_hook() called w/ prefix: {prefix}")
        ...
        for model_name, stride in param_strides.items():
            chkpt_name = prefix + model_name if prefix not in model_name else model_name
