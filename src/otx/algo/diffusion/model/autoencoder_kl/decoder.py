...
class UpBlock(nn.Module):
    ...
    def __init__(self, in_channels: int, out_channels: int, is_first: bool = False):
        ...
        if not is_first:
            # layer names are important for checkpoint loading
            self.upsample = nn.ModuleDict({"conv": nn.Conv2d(out_channels, out_channels, 3, padding=1)})
