...
from otx.algo.object_detection_3d.utils.misc import (
    accuracy,
    inverse_sigmoid,
)
...
def tgt_loss_boxes(src_boxes, tgt_boxes, num_tgt):
    ...
    loss_giou = 1 - torch.diag(
        box_ops.generalized_box_iou(box_ops.box_cxcylrtb_to_xyxy(src_boxes), box_ops.box_cxcylrtb_to_xyxy(tgt_boxes)),
    )
...
def tgt_loss_labels(src_logits_, tgt_labels_, num_tgt, focal_alpha, log=True):
    ...
    loss_ce = (
        sigmoid_focal_loss(src_logits, target_classes_onehot, num_tgt, alpha=focal_alpha, gamma=2) * src_logits.shape[1]
    )
...
def compute_dn_loss(mask_dict, training, aux_num, focal_alpha):
    ...
    if training and "output_known_lbs_bboxes" in mask_dict:
        ...
        losses["tgt_loss_center"] = torch.as_tensor(0.0).to("cuda")
        # losses['tgt_loss_depth'] = torch.as_tensor(0.).to('cuda')
        # losses['tgt_loss_dim'] = torch.as_tensor(0.).to('cuda')
        losses["tgt_loss_angle"] = torch.as_tensor(0.0).to("cuda")
    ...
    if aux_num:
        for i in range(aux_num):
            ...
            if training and "output_known_lbs_bboxes" in mask_dict:
                ...
                l_dict = {k + f"_{i}": v for k, v in l_dict.items()}
                losses.update(l_dict)
                l_dict = tgt_loss_angles(
                    output_known_angle[-1],
                    known_target_heading_cls,
                    known_target_heading_res,
                    num_tgt,
                )
                ...
                l_dict["tgt_loss_center"] = torch.as_tensor(0.0).to("cuda")
                l_dict["tgt_loss_angle"] = torch.as_tensor(0.0).to("cuda")
                l_dict = {k + f"_{i}": v for k, v in l_dict.items()}
                losses.update(l_dict)
    return losses
