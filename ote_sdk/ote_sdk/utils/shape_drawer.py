...
class Helpers:
    ...
    def generate_text_for_label(
        ...
        if show_confidence and isinstance(label, ScoredLabel):
            if len(text) > 0:
                text += " "
            text += f"{label.probability:.0%}" if label.probability <= 1.0 else f"{label.probability:.0f}%"
        return text
...
class ShapeDrawer(DrawerEntity[AnnotationSceneEntity]):
    ...
    def draw(
        ...
        if not isinstance(entity, NullAnnotationSceneEntity):
            ...
            if self.show_count:
                image = self.top_left_drawer.draw_annotation_count(
                    image, num_annotations
                )
        return image
    ...
    class PolygonDrawer(Helpers, DrawerEntity[Polygon]):
        ...
        def draw(
            self, image: np.ndarray, entity: Polygon, labels: List[ScoredLabel]
        ) -> np.ndarray:
            base_color = labels[0].color.bgr_tuple
            ...
            contours = np.array(
                [
                    [point.x * image.shape[1], point.y * image.shape[0]]
