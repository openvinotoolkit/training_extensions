...
class RSNAInference():
    ...
    def test(
            model, dataLoaderTest, nnClassCount,
        ...
        aurocIndividual = RSNATrainer.computeAUROC(
            tfunc.one_hot(
                outGT.squeeze(1).long()).float(),
            outPRED, nnClassCount)
...
def main(args):
    ...
    batch, losst, losse = RSNAInference.test(
        model, dataLoaderTest, nnClassCount,
        checkpoint, class_names, device)
