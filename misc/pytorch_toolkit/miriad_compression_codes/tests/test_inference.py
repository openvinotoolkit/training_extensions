def create_inference_test_for_phase():
    class InferenceTest(unittest.TestCase):
        ...
        def test_pytorch_inference(self):
            inference = CustomDatasetPhase1(dataloader_test=self.tst_loader, checkpoint=self.model_path, device=self.device)
        ...
        def test_ir_inference(self):
            inference = CustomDatasetPhase1(dataloader_test=self.tst_loader, checkpoint=self.onnx_model_path, device='cpu')
