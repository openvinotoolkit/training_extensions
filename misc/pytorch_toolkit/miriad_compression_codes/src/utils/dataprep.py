	...
	try:
		for root, folders, files in os.walk(os.path.abspath(args.path_to_dataset)):
			...
			if len(files) == 1:
				...
				patient_id = re.findall(r"\D(P_\d{5})\D", dcm_path)[0] # search for 'P_?????' pattern
				...
				if image.dtype == 'uint16' and \
					...
					if is_training:
						...
						patches = view_as_windows(image, window_shape=(args.patch_dim, args.patch_dim), step=args.patch_dim)
