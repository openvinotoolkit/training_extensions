...
def mse(preds: Tensor, target: Tensor):
    return ((preds - target) ** 2).reshape(preds.size(0), -1).mean(dim=-1)
...
def psnr(img1: Tensor, img2: Tensor, epsilon=1e-8, val_range=None):
    if val_range is None:
        if torch.max(img1) > 128:
            max_val = 255
        else:
    ...
    return 10 * torch.log10(val_range ** 2 / (mse(img1, img2) + epsilon))
