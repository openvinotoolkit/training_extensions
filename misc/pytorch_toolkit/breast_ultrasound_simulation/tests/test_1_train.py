...
class TestTrain(unittest.TestCase):
    ...
    def test_train(self):
        ...
        training_set = Kaggle_BUS_Dataset(all_files, self.data_config['stage0_data'],
                          self.data_config['realUS_data'], resize=True)
        training_gen = data.DataLoader(dataset=training_set, **self.params)
        solver_ins = solver(self.train_config, self.gen_config, training_gen, restore=self.train_config['restore'])
        l1_loss_epoch = solver_ins.train()
        self.assertLessEqual(l1_loss_epoch[-1], l1_loss_epoch[0])
