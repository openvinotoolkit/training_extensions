...
class SpectralConv2d(nn.Module):
    def __init__(self, ch, k, p, py, s, dil):
        ...
        self.ydim = nn.Conv2d(
            ch, ch, (k, 1), padding=(py, 0), stride=(s, 1), dilation=(dil, 1), bias=False)
        self.xdim = nn.Conv2d(
            ch, ch, (1, k), padding=(0, p), stride=(1, s), groups=ch, bias=False)
        self.bias = nn.Conv2d(
            ch, ch, (1, 1), padding=(0, 0), stride=(1, 1), groups=ch, bias=True)
...
class SpectralConv2dInter(nn.Module):
    def __init__(self, ch, k, pad, dil, wts_list=None, a=1):
        ...
        self.pad = pad
        index = torch.Tensor(
            list(range(0, self.kernel, dil))).to(torch.int64)
        ...
        self.xdim = nn.Conv2d(
            ch, ch, (1, k), padding=(0, 1), stride=(1, 1), groups=ch, bias=False)
        self.bias = nn.Conv2d(
            ch, ch, (1, 1), padding=(0, 0), stride=(1, 1), groups=ch, bias=True)
...
class SpectralConv3dInter(nn.Module):
    def __init__(self, ch, k, pad, dil):
        ...
        p = int((pad == 2))
        self.ydim = nn.Conv3d(
            ch, ch, (1, self.kernel, 1), padding=(0, pad, 0), stride=(1, 1, 1), bias=False)
        self.xdim = nn.Conv3d(
            ch, ch, (1, 1, k), padding=(0, 0, p), stride=(1, 1, 1), groups=ch, bias=False)
        self.zdim = nn.Conv3d(
            ch, ch, (k, 1, 1), padding=(p, 0, 0), stride=(1, 1, 1), groups=ch, bias=False)
        self.bias = nn.Conv3d(
            ch, ch, (1, 1, 1), padding=(0, 0, 0), stride=(1, 1, 1), groups=ch, bias=True)
...
class GeneratorInter(nn.Module):
    def __init__(self, in_ch, model=None, a=1):
        ...
        self.down1 = nn.Sequential(
            ...
            nn.LeakyReLU(inplace=True),
            # 3x3 spectral conv
            SpectralConv2dInter(32, k=3, pad=2, dil=2, wts_list=model.down1[-3], a=a),
            nn.BatchNorm2d(32),
            nn.LeakyReLU(inplace=True)
        )
