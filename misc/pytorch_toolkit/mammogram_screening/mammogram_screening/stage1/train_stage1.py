...
from ..train_utils.val_function import val_stage1, val_stage2
...
if __name__ == '__main__':
    ...
    optimizer = optim.SGD(filter(lambda p: p.requires_grad, model.parameters()), lr=l_rate, momentum=0.9, weight_decay=0.0005)
    ...
    for epoch in tq(range(epochs)):
        train_loss_bce, train_loss_dice, train_dice = train_stage1(model, train_loader, optimizer, epoch, epochs, device, verbose=True)
        ...
        val_loss_bce, val_loss_dice, val_dice = val_stage1(model, val_loader, epoch, epochs, device, verbose=True, save_qual=False, save_pth='')
        scheduler.step()
