...
def generate_negative_patch(jsonpath, fold, data_path, lung_segpath, savepath, category='train_set'):
    ...
    for img_name in tq(nm_list):
        ...
        mask = np.load(os.path.join(mask_dir, img_name)
                       ).astype(np.uint8)  # *255

        if np.any(mask):
            # Convert grayscale image to binary
            # parameters are ip_img,threshold,max_value
            _, th_mask = cv2.threshold(mask, 0.5, 1, 0, cv2.THRESH_BINARY)
            contours, _ = cv2.findContours(
                th_mask, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)
                ...
                for _ in range(patch_count):
                    ...
                    try:
                        ...
                        x, y = random.randrange(
                            xr, xr+wr-size/2), random.randrange(yr, yr+hr-size/2)

                    except:
                        prob = random.randrange(0, 1)
                        if prob > 0.5:
                            x, y = random.randrange(
                                xr, xr+wr/2), random.randrange(yr, yr+hr/2)
                        else:
                            x, y = random.randrange(
                                int(xr+wr/2), xr+wr), random.randrange(int(yr+hr/2), yr+hr)

                    if x+size < 512 & y+size < 512:
                        patch_img = img[y: y+size, x: x +
                                        size].copy().astype(np.float16)
                        patch_mask = np.zeros((size, size)).astype(np.float16)

                    else:
                        if x-size <= 0 & y-size <= 0:
                            patch_img = img[0: size, 0: size].copy().astype(
                                np.float16)
                            patch_mask = np.zeros(
                                (size, size)).astype(np.float16)

                        elif x-size <= 0 & y-size > 0:
                            patch_img = img[y-size: y,
                                            0: size].copy().astype(np.float16)
                            patch_mask = np.zeros(
                                (size, size)).astype(np.float16)

                        elif x-size > 0 & y-size <= 0:
                            patch_img = img[0: size, x -
                                            size: x].copy().astype(np.float16)
                            patch_mask = np.zeros(
                                (size, size)).astype(np.float16)

                        else:

                            patch_img = img[y-size: y, x -
                                            size: x].copy().astype(np.float16)
                            patch_mask = np.zeros(
                                (size, size)).astype(np.float16)
                    ...
                    if not os.path.isdir(img_savepath):
                        ...
                        np.save(img_savepath+'patch_'+str(fold) +
                                '_'+str(index)+'.npy', patch_img)
                    else:
                        np.save(img_savepath+'patch_'+str(fold) +
                                '_'+str(index)+'.npy', patch_img)
                    ...
                    if not os.path.isdir(mask_savepath):
                        ...
                        np.save(mask_savepath+'patch_'+str(fold) +
                                '_'+str(index)+'.npy', patch_mask)
                    else:
                        np.save(mask_savepath+'patch_'+str(fold) +
                                '_'+str(index)+'.npy', patch_mask)
            ...
            for cntr in contours:
                ...
                if int(yc-size/2) < 0 or int(xc-size/2) < 0:
                    if int(yc-size/2) < 0 and int(xc-size/2) < 0:
                        patch_img1 = img[0:size, 0:size].copy().astype(
                            np.float16)
                        patch_mask1 = mask[0:size,
                                           0:size].copy().astype(np.float16)

                    elif int(yc-size/2) > 0 and int(xc-size/2) < 0:
                        patch_img1 = img[int(
                            yc-size/2):int(yc+size/2), 0:size].copy().astype(np.float16)
                        patch_mask1 = mask[int(
                            yc-size/2):int(yc+size/2), 0:size].copy().astype(np.float16)

                    elif int(yc-size/2) < 0 and int(xc-size/2) > 0:
                        patch_img1 = img[0:size, int(
                            xc-size/2):int(xc+size/2)].copy().astype(np.float16)
                        patch_mask1 = mask[0:size, int(
                            xc-size/2):int(xc+size/2)].copy().astype(np.float16)
                    ...
                    if int(yc+size/2) > 512 and int(xc+size/2) > 512:
                        ...
                        n = xc + size - 512
                        patch_img1 = img[int(
                            yc-m):512, int(xc-n):512].copy().astype(np.float16)
                        patch_mask1 = mask[int(
                            yc-m):512, int(xc-n):512].copy().astype(np.float16)
                        ...
                        m = yc+size - 512
                        patch_img1 = img[int(
                            yc-m):512, int(xc-size/2):int(xc+size/2)].copy().astype(np.float16)
                        patch_mask1 = mask[int(
                            yc-m):512, int(xc-size/2):int(xc+size/2)].copy().astype(np.float16)
                        ...
                        n = xc+size - 512
                        patch_img1 = img[int(
                            yc-size/2):int(yc+size/2), int(xc-n):512].copy().astype(np.float16)
                        patch_mask1 = mask[int(
                            yc-size/2):int(yc+size/2), int(xc-n):512].copy().astype(np.float16)
                ...
                for i in range(patch_count):
                    ...
                    if i == 0:
                        ...
                        if xc+size < 512 and yc+size < 512:
                            patch_img = img[yc:yc+size, xc:xc +
                                            size].copy().astype(np.float16)
                            patch_mask = mask[yc:yc+size, xc:xc +
                                              size].copy().astype(np.float16)

                        elif xc+size > 512 and yc+size < 512:
                            m = xc+size-512
                            patch_img = img[yc:yc+size, xc-m:xc +
                                            size-m].copy().astype(np.float16)
                            patch_mask = mask[yc:yc+size, xc -
                                              m:xc+size-m].copy().astype(np.float16)

                        elif xc+size < 512 and yc+size > 512:
                            n = yc+size-512
                            patch_img = img[yc-n:yc+size-n,
                                            xc:xc+size].copy().astype(np.float16)
                            patch_mask = mask[yc-n:yc+size-n,
                                              xc:xc+size].copy().astype(np.float16)
                        else:
                            m = xc+size-512
                            n = yc+size-512
                            patch_img = img[yc-n:yc+size-n, xc -
                                            m:xc+size-m].copy().astype(np.float16)
                            patch_mask = mask[yc-n:yc+size-n, xc -
                                              m:xc+size-m].copy().astype(np.float16)
                    elif i == 1:

                        if xc-size > 0 and yc+size < 512:
                            patch_img = img[yc:yc+size, xc -
                                            size:xc].copy().astype(np.float16)
                            patch_mask = mask[yc:yc+size, xc -
                                              size:xc].copy().astype(np.float16)

                        elif xc-size < 0 and yc+size < 512:

                            patch_img = img[yc:yc+size,
                                            0:size].copy().astype(np.float16)
                            patch_mask = mask[yc:yc+size,
                                              0:size].copy().astype(np.float16)
                            ...
                            patch_img = img[yc-n:yc+size-n, xc -
                                            size:xc].copy().astype(np.float16)
                            patch_mask = mask[yc-n:yc+size-n,
                                              xc-size:xc].copy().astype(np.float16)
                        ...
                        if xc+size < 512 and yc-size > 0:
                            patch_img = img[yc-size:yc, xc:xc +
                                            size].copy().astype(np.float16)
                            patch_mask = mask[yc-size:yc, xc:xc +
                                              size].copy().astype(np.float16)

                        elif xc+size > 512 and yc-size > 0:
                            m = xc+size-512
                            patch_img = img[yc-size:yc, xc-m:xc +
                                            size-m].copy().astype(np.float16)
                            patch_mask = mask[yc-size:yc, xc -
                                              m:xc+size-m].copy().astype(np.float16)

                        elif xc+size < 512 and yc-size < 0:
                            patch_img = img[0:size, xc:xc +
                                            size].copy().astype(np.float16)
                            patch_mask = mask[0:size, xc:xc +
                                              size].copy().astype(np.float16)
                        ...
                        if xc-size > 0 and yc-size > 0:
                            ...
                            m = xc+size-512
                            patch_img = img[yc-size:yc,
                                            0:size].copy().astype(np.float16)
                            patch_mask = mask[yc-size:yc,
                                              0:size].copy().astype(np.float16)

                        elif xc-size > 0 and yc-size < 0:
                            patch_img = img[0:size, xc -
                                            size:xc].copy().astype(np.float16)
                            patch_mask = mask[0:size, xc -
                                              size:xc].copy().astype(np.float16)

                        else:
                            patch_img = img[0:size, 0:size].copy().astype(
                                np.float16)
                            patch_mask = mask[0:size,
                                              0:size].copy().astype(np.float16)
