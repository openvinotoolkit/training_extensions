...
import torch.nn
from ..models.build import build_or_load_model
from ..utils.dataset import WholeVolumeSegmentationDataset
...
def run_pytorch_training(config, model_filename, training_log_filename, verbose=1, use_multiprocessing=False,
    ...
    model = build_or_load_model(model_name, model_filename, n_features=config["n_features"], n_outputs=n_outputs,
                                ...
                                bias=bias, n_gpus=n_gpus, tsm_length=window[-1], **model_kwargs)
    model_parameter = filter(lambda p:p.requires_grad, model.parameters())
    ...
    # 4. Create datasets
    # print(config['training_filenames'])
    ...
    if 'skip_validation' in config and config['skip_validation']:
        ...
        # print(config['validation_filenames'])
...
def train(model, optimizer, criterion, n_epochs, training_loader, validation_loader, training_log_filename,
    ...
    if os.path.exists(training_log_filename):
        ...
        start_epoch = 0
    training_log_header = ["epoch", "loss", "lr", "val_loss", "val_dice"]
    ...
    for epoch in range(start_epoch, n_epochs):
        ...
        if validation_loader:
            val_loss, val_dice = epoch_validatation(validation_loader, model, criterion, n_gpus=n_gpus, regularized=regularized,
                                          vae=vae)
        else:
            val_loss = None
            val_dice = None
        ...
        training_log.append([epoch, loss, get_lr(optimizer), val_loss, val_dice])
