...
from src.utils.losses import compute_per_channel_dice
...
def batch_loss_eval(model, images, target, criterion, n_gpus=0, regularized=False, vae=False):
    if n_gpus is not None:
        images = images.cuda()
        target = target.cuda()
    # compute output
    ...
    if regularized:
        ...
        loss = criterion(output, target)
        dice = compute_per_channel_dice(output, target)
    return loss, dice, batch_size
...
def epoch_validatation(val_loader, model, criterion, n_gpus, print_freq=1, regularized=False, vae=False):
    ...
    with torch.no_grad():
        ...
        for i, (images, target) in enumerate(val_loader):
            ...
            loss, dice, batch_size = batch_loss_eval(model, images, target, criterion, n_gpus=n_gpus, regularized=regularized,
                                          vae=vae)

            # measure accuracy and record loss
            losses.update(loss.item(), batch_size)
            if(i==0):
                dices =  dice.cpu().numpy()
            else:
                dices += dice.cpu().numpy()
            # measure elapsed time
            batch_time.update(time.time() - end)
            ...
            if i % print_freq == 0:
                progress.display(i)
                print("Dice Coefficient ", dice.cpu().numpy())
    print("The average dice score is ", dices/len(val_loader))
    return losses.avg, dices/len(val_loader)
