            ...
            for command in commands:
                ...
                if help_message not in full_text:
                    ...
                    for _ in range(MAX_INDENT):
                        help_message = "\n".join(
                            [" " + line for line in help_message.split("\n")]
                        )
    ...
    def test_algorithms_table(self):
        def algorithms_generate_table(templates):
            ...
            attributes_in_md = {
                ...
                "gigaflops": "Complexity (GFlops)",
                "size": "Model size (MB)",
                "Path": "Path",
            }

            table = [" | ".join([attributes_in_md[x] for x in header])] + [
            ...
            for template in sorted(templates, key=lambda x: float(x.gigaflops)):
                ...
                record.append(
                    os.path.relpath(template.model_template_path, "./external")
                )
                record = " | ".join(record)
                table += [record]
            return "\n".join(table)
