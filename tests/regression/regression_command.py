    ...
    for k in trained_performance.keys():
        if isinstance(criteria, dict) and template.name in criteria.keys():
            ...
            if(exported_performance[k] < modified_criteria): 
                regression_result["passed"] = False
                regression_result["log"] = f"Export performance: ({exported_performance[k]}) < Criteria: ({modified_criteria})."

        if(exported_performance[k] < trained_performance[k] or 
...
def regression_deployment_testing(
    ...
    for k in exported_performance.keys():
        if isinstance(criteria, dict) and template.name in criteria.keys():
            ...
            if(exported_performance[k] < modified_criteria):
                regression_result["passed"] = False 
                regression_result["log"] = f"Deploy performance: ({deployed_performance[k]}) < Criteria: ({modified_criteria})."
        if(
    ...
    for k in trained_performance.keys():
        if isinstance(criteria, dict) and template.name in criteria.keys():
            ...
            if(evaluated_performance[k] < modified_criteria):
                regression_result["passed"] = False
                regression_result["log"] = f"NNCF performance: ({evaluated_performance[k]}) < Criteria: ({modified_criteria})."
        if(
