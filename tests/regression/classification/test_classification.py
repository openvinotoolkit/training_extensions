...
class TestRegressionMultiClassClassification:
    ...
    def test_ptq_optimize_eval(self, template, tmp_dir_path):
        ...
        ptq_optimize_testing(template, tmp_dir_path, otx_dir, multi_class_data_args)
        ...
        test_result = regression_ptq_eval_testing(
            ...
            otx_dir,
            multi_class_data_args,
            criteria=multi_class_regression_config["regression_criteria"]["ptq"],
            reg_threshold=0.10,
            result_dict=self.performance[template.name],
        )
...
class TestRegressionMultiLabelClassification:
    ...
    def test_ptq_optimize_eval(self, template, tmp_dir_path):
        ...
        tmp_dir_path = tmp_dir_path / "multi_label_cls"
        ptq_start_time = timer()
        ptq_optimize_testing(template, tmp_dir_path, otx_dir, multi_label_data_args)
        ptq_elapsed_time = timer() - ptq_start_time
        ...
        test_result = regression_ptq_eval_testing(
            ...
            otx_dir,
            multi_label_data_args,
            criteria=multi_label_regression_config["regression_criteria"]["ptq"],
            reg_threshold=0.10,
            result_dict=self.performance[template.name],
        )
        ptq_eval_elapsed_time = timer() - ptq_eval_start_time
...
class TestRegressionHierarchicalLabelClassification:
    ...
    def test_ptq_optimize_eval(self, template, tmp_dir_path):
        ...
        tmp_dir_path = tmp_dir_path / "h_label_cls"
        ptq_start_time = timer()
        ptq_optimize_testing(template, tmp_dir_path, otx_dir, h_label_data_args)
        ptq_elapsed_time = timer() - ptq_start_time
        ...
        test_result = regression_ptq_eval_testing(
            ...
            otx_dir,
            h_label_data_args,
            criteria=h_label_regression_config["regression_criteria"]["ptq"],
            reg_threshold=0.10,
            result_dict=self.performance[template.name],
        )
        ptq_eval_elapsed_time = timer() - ptq_eval_start_time
