...
class TestRegressionAnomalyClassification:
    ...
    def test_otx_train(self, template, tmp_dir_path):
        ...
        otx_train_testing(template, tmp_dir_path, otx_dir, anomaly_classification_data_args)
        train_elapsed_time = timer() - train_start_time
        
        infer_start_time = timer()
        otx_eval_compare(
            template, tmp_dir_path, otx_dir, anomaly_classification_data_args, 
            anomaly_classification_regression_config["regression_criteria"]["train"], 
            self.performance[template.name],
            self.acc_metric
        )
    ...
    def test_otx_export_eval_openvino(self, template, tmp_dir_path):
        ...
        otx_eval_openvino_testing(
            ...
            tmp_dir_path,
            otx_dir,
            anomaly_classification_data_args,
            threshold=1.0,
            criteria=anomaly_classification_regression_config["regression_criteria"]["export"],
            reg_threshold=0.10,
    ...
    def test_otx_deploy_eval_deployment(self, template, tmp_dir_path):
        ...
        otx_deploy_openvino_testing(template, tmp_dir_path, otx_dir, anomaly_classification_data_args)
        ...
        otx_eval_deployment_testing(
            ...
            anomaly_classification_data_args,
            threshold=1.0,
            criteria=anomaly_classification_regression_config["regression_criteria"]["deploy"],
            reg_threshold=0.10,
            result_dict=self.performance[template.name],
            acc_metric=self.acc_metric
        )
    ...
    def test_nncf_optimize_eval(self, template, tmp_dir_path):
        ...
        nncf_optimize_testing(template, tmp_dir_path, otx_dir, anomaly_classification_data_args)
        ...
        nncf_eval_testing(
            ...
            anomaly_classification_data_args,
            threshold=1.0,
            criteria=anomaly_classification_regression_config["regression_criteria"]["nncf"],
            reg_threshold=0.10,
            result_dict=self.performance[template.name],
            acc_metric=self.acc_metric
        )
    ...
    def test_pot_optimize_eval(self, template, tmp_dir_path):
        ...
        pot_optimize_testing(template, tmp_dir_path, otx_dir, anomaly_classification_data_args)
