...
segmentation_data_args["--gpus"] = "0,1"
...
class TestRegressionSegmentation:
    ...
    def test_ptq_optimize_eval(self, template, tmp_dir_path):
        ...
        ptq_optimize_testing(template, tmp_dir_path, otx_dir, segmentation_data_args)
        ...
        test_result = regression_ptq_eval_testing(
            ...
            otx_dir,
            segmentation_data_args,
            criteria=segmentation_regression_config["regression_criteria"]["ptq"],
            reg_threshold=0.10,
            result_dict=self.performance[template.name],
        )
