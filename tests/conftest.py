...
from otx.core.data.entity.visual_prompting import VisualPromptingBatchDataEntity, VisualPromptingBatchPredEntity, VisualPromptingDataEntity
...
def fxt_vpm_data_entity() -> tuple[tuple, VisualPromptingDataEntity, VisualPromptingBatchDataEntity]:
    ...
    fake_image = torch.rand(img_size)
    fake_image_info = ImageInfo(img_idx=0, img_shape=img_size, ori_shape=img_size)
    fake_bboxes = tv_tensors.BoundingBoxes([[0, 0, 1, 1]], format=tv_tensors.BoundingBoxFormat.XYXY, canvas_size=img_size, dtype=torch.float32)
    fake_masks = torch.rand(img_size)
    fake_labels = torch.as_tensor([1], dtype=torch.int64)
    fake_polygons = [None]
    # define data entity
    single_data_entity = VisualPromptingDataEntity(fake_image, fake_image_info, fake_bboxes, fake_masks, fake_labels, fake_polygons)
    batch_data_entity = VisualPromptingBatchDataEntity(1, [fake_image], [fake_image_info], [fake_bboxes], [fake_masks], [fake_labels], [fake_polygons])
    batch_pred_data_entity = VisualPromptingBatchPredEntity(1, [fake_image], [fake_image_info], [], [fake_bboxes], [fake_masks], [fake_labels], [fake_polygons])
