...
# Classification Integration Test Dataset
DATASET_PATH = "tests/assets/classification_dataset"
PREDICTION_SAMPLE = "tests/assets/classification_dataset/0/11.jpg"
...
class TestMMPretrainAPI:
    """
    This class contains integration tests for the training API of the MMClassification library.
    ...
    def dataset(self) -> Dataset:
            ...
            """
            return Dataset(
                train_data_roots=DATASET_PATH,
                val_data_roots=DATASET_PATH,
                test_data_roots=DATASET_PATH,
            )
    ...
    def test_dataset_api(self, dataset: Dataset) -> None:
            ...
            Returns:
                None
            """
            assert_torch_dataset_api_is_working(dataset=dataset, data_size=25)
    ...
    def test_engine_api(self, dataset: Dataset, model: str, tmp_dir_path: Path) -> None:
        ...
        # Validation
        val_score = engine.validate(batch_size=1)
        assert "accuracy/top1" in val_score
        assert val_score["accuracy/top1"] > 0.0

        # Test
        test_score = engine.test(test_dataloader=dataset.test_dataloader(), batch_size=1)
