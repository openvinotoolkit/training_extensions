...
def fxt_data_list() -> list:
    ...
    for _ in range(num_data):
        ...
        key = "".join(
            [string.ascii_lowercase[i] for i in rng.integers(0, len(string.ascii_lowercase), size=[key_len])],
        )
        meta = {
            "key": key,
        }
...
class TestMemCacheHandler:
    ...
    def test_cpu_limits(self, mode: str) -> None:
    ...
    @pytest.mark.parametrize("mode", ["singleprocessing", "multiprocessing"])
    def test_fully_caching(self, mode: str, fxt_data_list: list) -> None:
    ...
    @pytest.mark.parametrize("mode", ["singleprocessing", "multiprocessing"])
    def test_unfully_caching(self, mode: str, fxt_data_list: list) -> None:
