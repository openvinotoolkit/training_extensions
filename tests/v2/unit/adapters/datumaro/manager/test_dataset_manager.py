...
from otx.v2.api.utils.auto_utils import TASK_TYPE_TO_SUPPORTED_FORMAT
...
class TestOTXDatasetManager:
    ...
    def test_get_train_dataset(self, task: List[str], subset: List[str]) -> None:
        if subset == "val":
    ...
    @pytest.mark.parametrize("task", AVAILABLE_TASKS)
    @pytest.mark.parametrize("subset", AVAILABLE_SUBSETS)
    def test_get_val_dataset(self, task: List[str], subset: List[str]) -> None:
        if subset == "train":
    ...
    def test_get_data_format(self, data_root: str) -> None:
        assert isinstance(DatasetManager.get_data_format(data_root), str)

    @pytest.mark.parametrize("task", AVAILABLE_TASKS)
    @pytest.mark.parametrize("subset", AVAILABLE_SUBSETS)
    def test_get_image_path(self, task: str, subset: str) -> None:
    ...
    @pytest.mark.parametrize("task", AVAILABLE_TASKS)
    @pytest.mark.parametrize("subset", AVAILABLE_SUBSETS)
    def test_export_dataset(self, task: str, subset: str, tmp_dir_path: Path) -> None:
    ...
    def test_import_dataset(self, data_root: str) -> None:
    ...
    def test_import_dataset_with_direct_annotation(self, data_root: str) -> None:
        data_format = DATA_ROOTS2FORMAT[data_root]
        assert DatasetManager.import_dataset(data_root, data_format=data_format) is not None
    ...
    @pytest.mark.parametrize("task", AVAILABLE_TASKS)
    @pytest.mark.parametrize("subset", AVAILABLE_SUBSETS)
    def test_auto_split(self, task: str, subset: str) -> None:
