...
class TestBsSearchAlgo:
    ...
    def get_mock_train_func(self, cuda_oom_bound: int, max_runnable_bs: int) -> Callable:
    ...
    def test_try_batch_size(self) -> None:
    ...
    def test_try_batch_size_cuda_oom(self) -> None:
    ...
    def _prepare_dist_test(self, broadcast_val: torch.Tensor, gather_val: Optional[List[torch.Tensor]] = None) -> None:
        self.mock_dist.is_initialized.return_value = True

        # mocking torch.distributed.broadcast
        def mock_broadcast(tensor: torch.Tensor, src: int) -> None:
            _ = src
        ...
        def mock_gather(tensor: torch.Tensor, gather_list: Optional[List[torch.Tensor]] = None, dst: int = 0) -> None:
            _, _ = tensor, dst
            if gather_list is not None and gather_val is not None:
        ...
        def mock_tensor_cuda(self, *args, **kwargs) -> "TestBsSearchAlgo":
            _, _ = args, kwargs
            return self
    ...
    def test_try_batch_size_distributed_not_rank_0(self) -> None:
        ...
        assert not self.mock_dist.gather.call_args.args[0][0].item()
    ...
    def test_try_batch_size_distributed_rank_0(self) -> None:
        ...
        self.mock_dist.broadcast.assert_called_once()
        assert self.mock_dist.broadcast.call_args.kwargs["src"] == 0
        assert self.mock_dist.broadcast.call_args.args[0][0].item()
