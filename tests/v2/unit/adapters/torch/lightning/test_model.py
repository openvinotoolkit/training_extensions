...
from otx.v2.adapters.torch.lightning.model import get_model, list_models
...
def test_get_model(mocker: MockerFixture, monkeypatch: MonkeyPatch, tmp_dir_path: Path) -> None:
    ...
    monkeypatch.setattr(
        "otx.v2.adapters.torch.lightning.model.MODEL_CONFIGS",
        {
            "SAM_tiny": str(sam_tiny_config_file),
        },
    )
    monkeypatch.setattr(
        "otx.v2.adapters.torch.lightning.model.MODELS",
        registry,
    )
    ...
    # from checkpoint
    mock_load = mocker.patch("otx.v2.adapters.torch.lightning.model.torch.load", return_value={"model": {"state_dict": {}}})
