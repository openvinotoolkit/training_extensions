...
from otx.v2.adapters.torch.lightning.visual_prompt.modules.models.visual_prompters.segment_anything import (
    SegmentAnything,
)
...
class TestSegmentAnything:
    ...
    def setup(self, monkeypatch) -> None:
        monkeypatch.setattr(
            "otx.v2.adapters.torch.lightning.visual_prompt.modules.models.visual_prompters.segment_anything.SAMImageEncoder",
            MockImageEncoder,
        )
        monkeypatch.setattr(
            "otx.v2.adapters.torch.lightning.visual_prompt.modules.models.visual_prompters.segment_anything.SAMPromptEncoder",
            MockPromptEncoder,
        )
        monkeypatch.setattr(
            "otx.v2.adapters.torch.lightning.visual_prompt.modules.models.visual_prompters.segment_anything.SAMMaskDecoder",
            MockMaskDecoder,
        )
    ...
    def test_set_models(self, mocker, backbone: str) -> None:
        """Test set_models."""
        mocker.patch(
            "otx.v2.adapters.torch.lightning.visual_prompt.modules.models.visual_prompters.segment_anything.SegmentAnything.freeze_networks"
        )
        mocker.patch(
            "otx.v2.adapters.torch.lightning.visual_prompt.modules.models.visual_prompters.segment_anything.SegmentAnything.set_metrics"
        )
        mocker.patch(
            "otx.v2.adapters.torch.lightning.visual_prompt.modules.models.visual_prompters.segment_anything.SegmentAnything.load_checkpoint"
        )

        config = self.base_config.copy()
        config.model.update({"backbone": backbone})
        if backbone == "resnet":
    ...
    def test_set_metrics(self, mocker, loss_type: str) -> None:
        """Test set_metrics."""
        mocker.patch(
            "otx.v2.adapters.torch.lightning.visual_prompt.modules.models.visual_prompters.segment_anything.SegmentAnything.set_models"
        )
        mocker.patch(
            "otx.v2.adapters.torch.lightning.visual_prompt.modules.models.visual_prompters.segment_anything.SegmentAnything.freeze_networks"
        )
        ...
        config = self.base_config.copy()
        config.model.update({"loss_type": loss_type})
        sam = SegmentAnything(config)

        assert loss_type in ("sam", "medsam")
        if loss_type == "sam":
    ...
    def test_load_checkpoint_with_state_dict(self, mocker, is_backbone_arg: bool, state_dict: OrderedDict) -> None:
    ...
    def test_load_checkpoint_without_checkpoint(self, mocker) -> None:
        ...
        config = self.base_config.copy()
        config.model.update({"checkpoint": None})

        SegmentAnything(config, state_dict=None)

        assert True

    def test_load_checkpoint_with_url(self, mocker) -> None:
        ...
        mocker_load_state_dict = mocker.patch(
            "otx.v2.adapters.torch.lightning.visual_prompt.modules.models.visual_prompters.segment_anything.SegmentAnything.load_state_dict"
        )

        config = self.base_config.copy()
        config.model.update({"checkpoint": "http://checkpoint"})

        _ = SegmentAnything(config, state_dict=None)
    ...
    def test_load_checkpoint_from_local_checkpoint(self, mocker, checkpoint: str) -> None:
        ...
        mocker.patch("otx.v2.adapters.torch.lightning.visual_prompt.modules.models.visual_prompters.segment_anything.Path.open").__enter__.return_value = True
        mocker.patch("torch.load", return_value=OrderedDict())
        mocker_load_from_checkpoint = mocker.patch(
            "otx.v2.adapters.torch.lightning.visual_prompt.modules.models.visual_prompters.segment_anything.SegmentAnything.load_from_checkpoint"
        )
        mocker_load_state_dict = mocker.patch(
            "otx.v2.adapters.torch.lightning.visual_prompt.modules.models.visual_prompters.segment_anything.SegmentAnything.load_state_dict"
        )
        ...
        config = self.base_config.copy()
        config.model.update({"checkpoint": checkpoint})
    ...
    def test_embed_points(self, mocker, point_coords: Tensor, point_labels: Tensor, expected: tuple[int]) -> None:
        ...
        mock_pe_layer = mocker.patch.object(sam.prompt_encoder, "pe_layer")
        mock_pe_layer.pe_encoding.return_value = torch.empty((1, num_points, 4))
    ...
    def test_training_step(self, mocker, loss_type: str, expected: Tensor) -> None:
        """Test training_step."""
        mocker.patch(
            "otx.v2.adapters.torch.lightning.visual_prompt.modules.models.visual_prompters.segment_anything.SegmentAnything.forward",
            return_value=([torch.Tensor([[0, 1, 1, 0] for _ in range(4)])], [torch.tensor(1.0)]),
        )
        mocker.patch(
            "otx.v2.adapters.torch.lightning.visual_prompt.modules.models.visual_prompters.segment_anything.SegmentAnything.postprocess_masks",
    ...
    def test_training_epoch_end(self) -> None:
        ...
        sam = SegmentAnything(config=self.base_config)
        for v in sam.train_metrics.values():
    ...
    def test_validation_epoch_end(self) -> None:
        ...
        sam = SegmentAnything(config=self.base_config)
        for v in sam.val_metrics.values():
            v.update(torch.zeros((2, 2)), torch.zeros((2, 2), dtype=torch.int32))
    ...
    def test_predict_step(self, mocker, return_logits: bool, expected: Tensor) -> None:
        ...
        config = self.base_config.copy()
        config.model.update({"return_logits": return_logits})
        sam = SegmentAnything(config=config)
        batch = {
