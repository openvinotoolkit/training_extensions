...
def test_get_requirements_from_file(requirements_file: Path) -> None:
    """Test that get_requirements_from_file returns the expected list of requirements."""
    expected_requirements = ["numpy==1.19.5", "opencv-python-headless>=4.5.1.48"]
    requirements = get_requirements_from_file(str(requirements_file))
    assert isinstance(requirements, list)
    assert len(requirements) == len(expected_requirements)
    for req, expected_req in zip(requirements, expected_requirements):
        assert str(req) == expected_req
...
def test_get_requirements(mocker: MockerFixture) -> None:
    ...
    mocker.patch("otx.v2.cli.utils.install.requires", return_value=None)
...
def test_get_cuda_version_with_nvcc(mocker: MockerFixture) -> None:
    ...
    mock_run = mocker.patch("otx.v2.cli.utils.install.Path.exists", return_value=False)
...
def test_get_hardware_suffix(mocker: MockerFixture) -> None:
    mocker.patch("otx.v2.cli.utils.install.get_cuda_version", return_value="11.2")
    assert get_hardware_suffix() == "cu112"

    mocker.patch("otx.v2.cli.utils.install.get_cuda_version", return_value="12.1")
    ...
    mocker.patch("otx.v2.cli.utils.install.get_cuda_version", return_value=None)
...
def test_add_hardware_suffix_to_torch(mocker: MockerFixture) -> None:
    ...
    mocker.patch("otx.v2.cli.utils.install.get_hardware_suffix", return_value="cu121")
    ...
    mocker.patch("otx.v2.cli.utils.install.get_hardware_suffix", return_value="cu118")
...
def test_get_torch_install_args(mocker: MockerFixture) -> None:
    ...
    mocker.patch("otx.v2.cli.utils.install.platform.system", return_value="Linux")
    mocker.patch("otx.v2.cli.utils.install.get_hardware_suffix", return_value="cpu")
    ...
    requirement = Requirement.parse("torch>=1.13.0,<=2.0.1")
    mocker.patch("otx.v2.cli.utils.install.get_hardware_suffix", return_value="cu111")
    install_args = get_torch_install_args(requirement)
    expected_args = [
        "--extra-index-url",
    ...
    mocker.patch("otx.v2.cli.utils.install.platform.system", return_value="Darwin")
    requirement = Requirement.parse("torch==2.0.1")
    install_args = get_torch_install_args(requirement)
    assert install_args == ["torch==2.0.1"]

    mocker.patch("otx.v2.cli.utils.install.platform.system", return_value="Unknown")
...
def test_mim_installation(mocker: MockerFixture) -> None:
    mocker.patch("otx.v2.cli.utils.install.find_spec", return_value=True)
    ...
    mocker.patch("otx.v2.cli.utils.install.find_spec", return_value=False)
...
def test_get_module_version(mocker: MockerFixture) -> None:
    mock_get_distribution = mocker.patch("otx.v2.cli.utils.install.pkg_resources.get_distribution")
