...
from tests.integration.api.classification.test_api_classification import ClassificationTaskAPIBase, DEFAULT_CLS_TEMPLATE_DIR

from otx.algorithms.classification.utils import get_multihead_class_info
from otx.algorithms.classification.adapters.mmcls.data import MPAClsDataset, MPAMultilabelClsDataset, MPAHierarchicalClsDataset


class TestOTXClsDataset:
    @pytest.fixture(autouse=True)
    def setup(self) -> None:
        self.hyper_parameters, self.model_template = \
            ClassificationTaskAPIBase.setup_configurable_parameters(DEFAULT_CLS_TEMPLATE_DIR)
        self.dataset_len = 20
    ...
    def test_create_hierarchical_adapter(self):
        ...
        dataset = MPAHierarchicalClsDataset(otx_dataset=self.dataset,
                                            labels=self.dataset.get_labels(),
                                            hierarchical_info=class_info)
        assert dataset.num_classes == len(self.dataset.get_labels())
        assert len(dataset) == self.dataset_len * dataset.num_classes
        for d in dataset:
            assert d is not None
