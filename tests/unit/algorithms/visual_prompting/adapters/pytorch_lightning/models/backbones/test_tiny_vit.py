...
class TestDropPath:
    @e2e_pytest_unit
    def test_repr(self, drop_prob: float = 1.):
        """Test __repr__."""
...
class TestMBConv:
    ...
    def test_forward(self):
        """Test forward."""
        mbconv = MBConv(in_chans=3, out_chans=3, expand_ratio=1., activation=nn.Identity, drop_path=1.)
...
class TestBasicLayer:
    ...
    def test_extra_repr(self):
        """Test extra_repr."""
        assert f"dim={self.dim}, input_resolution={self.input_resolution}, depth={self.depth}" == self.basic_layer.extra_repr()
...
def test_build_tiny_vit(mocker):
    """Test build_tiny_vit."""
    mocker_vit = mocker.patch("otx.algorithms.visual_prompting.adapters.pytorch_lightning.models.backbones.tiny_vit.TinyViT")
