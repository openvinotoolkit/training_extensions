...
def test_run(mocker):
    ...
    model_cfg = OTXConfig.fromfile(os.path.join(DEFAULT_SEG_TEMPLATE_DIR, "model.py"))
...
def test_naive_export(mocker):
    exporter = SegmentationExporter()
    data_cfg = OTXConfig.fromfile(os.path.join(DEFAULT_SEG_TEMPLATE_DIR, "data_pipeline.py"))
