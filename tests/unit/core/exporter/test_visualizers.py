from otx.core.exporter.exportable_code.demo import BaseVisualizer, ClassificationVisualizer, InstanceSegmentationVisualizer, ObjectDetectionVisualizer, SemanticSegmentationVisualizer
import pytest
...
class TestClassificationVisualizer:
    ...
    def predictions(self):
        return ClassificationResult(top_labels=[(0, "cat", 0.9)], saliency_map=None, feature_vector=None, raw_scores=[0.9])
...
class TestDetectionVisualizer:
    @pytest.fixture
    def visualizer(self):
        return ObjectDetectionVisualizer(labels=["Pedestrian", "Car"], window_name="TestWindow", no_show=True, delay=10, output="test_output")
    ...
    def test_draw_with_predictions(self, visualizer):
        frame = np.zeros((100, 100, 3), dtype=np.uint8)
        predictions = DetectionResult([Detection(10, 40, 30, 80, 0.7, 2, "Car")], saliency_map=None, feature_vector=None)
        copied_frame = frame.copy()
        output_frame = visualizer.draw(frame, predictions)
...
class TestInstanceSegmentationVisualizer:
    @pytest.fixture
    def visualizer(self):
        return InstanceSegmentationVisualizer(labels=["person", "car"], window_name="TestWindow", no_show=True, delay=10, output="test_output")

    def test_draw_multiple_objects(self, visualizer):
        # Create a frame
        frame = np.zeros((100, 100, 3), dtype=np.uint8)
        copied_frame = frame.copy()

        # Create instance segmentation results with multiple objects
        predictions = InstanceSegmentationResult(segmentedObjects=[
            SegmentedObject(xmin=10, ymin=10, xmax=30, ymax=30, score=0.9, id=0, mask=np.random.randint(2, size=(100, 100), dtype=np.uint8), str_label='person'),
            SegmentedObject(xmin=40, ymin=40, xmax=60, ymax=60, score=0.8, id=1, mask=np.random.randint(2, size=(100, 100), dtype=np.uint8), str_label='car')
        ], saliency_map=None, feature_vector=None)
    ...
    def test_draw_empty_frame(self, visualizer):
        ...
        predictions = InstanceSegmentationResult(segmentedObjects=[
            SegmentedObject(xmin=10, ymin=10, xmax=30, ymax=30, score=0.9, id=0, mask=np.ones((20, 20)), str_label='person')
        ], saliency_map=None, feature_vector=None)
...
class TestSemanticSegmentationVisualizer:
    ...
    def visualizer(self, labels):
        return SemanticSegmentationVisualizer(labels=labels, window_name="TestWindow", no_show=True, delay=10, output="test_output")
    ...
    def test_apply_color_map(self, visualizer, labels):
        input_2d_mask = np.random.randint(0, len(labels), size=(10, 10))
        colored_mask = visualizer._apply_color_map(input_2d_mask)
        assert colored_mask.shape == (10, 10, 3)

    def test_draw(self, visualizer, labels):
        frame = np.random.randint(0, 255, size=(10, 10, 3), dtype=np.uint8)
        copied_frame = frame.copy()
        masks = ImageResultWithSoftPrediction(resultImage=np.random.randint(0, 255, size=(10, 10), dtype=np.uint8),
                                              soft_prediction=np.random.rand(10, 10), saliency_map=None, feature_vector=None)
