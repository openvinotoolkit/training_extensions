...
from tests.unit.core.data.test_helpers import generate_datumaro_dataset_item, generate_datumaro_dataset
...
class TestOTXDatasetManager:
    ...
    def test_get_val_dataset(self, task: List[str], subset: List[str]):
        if subset == "train":
            with pytest.raises(
                ValueError, 
                match=f"Can't find validation data."
            ):
                DatasetManager.get_val_dataset(self.dataset[subset][task])
        else:
            val_dataset = DatasetManager.get_val_dataset(
                self.dataset[subset][task]
            )
    ...
    def test_auto_split(self, task, subset):
        dataset = DatasetManager.auto_split(
            task=task,
            dataset=self.dataset[subset][task],
