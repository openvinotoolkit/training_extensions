"""Unit test for otx.algorithms.classification.adapters.mmcls.hooks.eval_hook."""
...
from otx.algorithms.classification.adapters.mmcls.hooks.eval_hook import (
    CustomEvalHook,
    DistCustomEvalHook,
    single_gpu_test,
)
...
class TestCustomEvalHook:
    ...
    def test_do_evaluate(self, mocker) -> None:
        ...
        hook = CustomEvalHook(metric="accuracy", dataloader=MockDataloader())
        runner = MockRunner()
        mocker.patch("otx.algorithms.classification.adapters.mmcls.hooks.eval_hook.single_gpu_test", return_value=[])
