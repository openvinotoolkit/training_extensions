...
from otx.mpa.det.semisl.inferrer import SemiSLDetectionInferrer
...
class TestSemiSLDetectionInferrer:
    @pytest.fixture(autouse=True)
    def setup(self) -> None:
        cfg = MPAConfig.fromfile(SEMISL_RECIPE_CONFIG_PATH)
        self.inferrer = SemiSLDetectionInferrer(name="", mode="train", config=cfg, common_cfg=None, index=0)
