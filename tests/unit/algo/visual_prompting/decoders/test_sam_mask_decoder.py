...
from otx.algo.visual_prompting.decoders.sam_mask_decoder import (
    MLP, Attention, SAMMaskDecoder, TwoWayAttentionBlock, TwoWayTransformer)
...
class TestMLP:
    ...
    def test_forward(self, inputs: torch.Tensor, expected: torch.Size):
        """Test forward."""
...
class TestTwoWayTransformer:
    ...
    @pytest.mark.parametrize(
        ...
        [(torch.empty((1, 8, 2, 2)), torch.empty((1, 8, 2, 2)), torch.empty((1, 1, 8)), (torch.Size((1, 1, 8)), torch.Size((1, 4, 8))))],
    )
    def test_forward(
        self,
...
class TestAttention:
    ...
    def test_separate_heads(self, x: torch.Tensor, expected: torch.Size):
    ...
    def test_recombine_heads(self, x: torch.Tensor, expected: torch.Size):
    ...
    @pytest.mark.parametrize(
        "q,k,v,expected", [(torch.empty((1, 1, 8)), torch.empty((1, 1, 8)), torch.empty((1, 1, 8)), torch.Size((1, 1, 8)))]
    )
    def test_forward(self, q: torch.Tensor, k: torch.Tensor, v: torch.Tensor, expected: torch.Size):
        """Test forward."""
