...
from otx.algo.visual_prompting.segment_anything import (OTXSegmentAnything,
                                                        SegmentAnything)
...
class TestSegmentAnything:
    ...
    def test_init(self, mocker, backbone: str, expected_backbone: str, freeze_image_encoder: bool, freeze_prompt_encoder: bool, freeze_mask_decoder: bool):
        """Test __init__."""
        mocker.patch("otx.algo.visual_prompting.segment_anything.SegmentAnything.load_state_dict")
        mocker_load_state_dict_from_url = mocker.patch("otx.algo.visual_prompting.segment_anything.torch.hub.load_state_dict_from_url")
        ...
        for param in segment_anything.image_encoder.parameters():
            assert param.requires_grad == (freeze_image_encoder == False)

        for param in segment_anything.prompt_encoder.parameters():
            assert param.requires_grad == (freeze_prompt_encoder == False)

        for param in segment_anything.mask_decoder.parameters():
            assert param.requires_grad == (freeze_mask_decoder == False)
            
    @pytest.mark.parametrize("training", [True, False])
    @pytest.mark.parametrize("ori_shapes",[[torch.tensor([512, 256])], [torch.tensor([256, 512])], [torch.tensor([1536, 1280])], [torch.tensor([1280, 1536])]])
    def test_forward(self, mocker, training: bool, ori_shapes: List[Tensor]) -> None:
        ...
        if training:
            ...
            assert all([isinstance(r[0], Tensor) for r in results])
    ...
    def test_postprocess_masks(
        self, input_size: int, orig_size: Tensor, expected: torch.Size
    ) -> None:
    ...
    @pytest.mark.parametrize("input_image_size,expected",
        [
            (torch.tensor((2, 4)), torch.tensor((3, 6))),
            (torch.tensor((4, 2)), torch.tensor((6, 3))),
        ]
    )
    def test_resize_longest_image_size(self, input_image_size: Tensor, expected: Tensor) -> None:
        """Test resize_longest_image_size."""
        segment_anything = SegmentAnything(backbone="tiny_vit")

        longest_side = 6

        results = segment_anything.resize_longest_image_size(input_image_size, longest_side)
...
class TestOTXSegmentAnything:
    ...
    def test_inspect_prompts(self, model):
        """Test _inspect_prompts."""
        # TODO: Add point prompts
        prompts: List[tv_tensors.BoundingBoxes] = [
            tv_tensors.BoundingBoxes([[0, 0, 1, 1]], format=tv_tensors.BoundingBoxFormat.XYXY, canvas_size=(2, 2), dtype=torch.float32),
