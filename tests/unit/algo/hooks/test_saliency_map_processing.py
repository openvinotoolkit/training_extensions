...
def test_process_all(postprocess) -> None:
    ...
    if postprocess:
        assert all(next(iter(s_map_dict.values())).shape == (OUT_SIZE, OUT_SIZE, 3) for s_map_dict in processed_saliency_maps)
    else:
        assert all(next(iter(s_map_dict.values())).shape == (RAW_SIZE, RAW_SIZE) for s_map_dict in processed_saliency_maps)
...
def test_process_predictions(postprocess) -> None:
    ...
    if postprocess:
        assert all(next(iter(s_map_dict.values())).shape == (OUT_SIZE, OUT_SIZE, 3) for s_map_dict in processed_saliency_maps if s_map_dict)
    else:
        assert all(next(iter(s_map_dict.values())).shape == (RAW_SIZE, RAW_SIZE) for s_map_dict in processed_saliency_maps if s_map_dict)
...
def test_process_image(postprocess) -> None:
    ...
    if postprocess:
        assert all(s_map_dict["map_per_image"].shape == (OUT_SIZE, OUT_SIZE, 3) for s_map_dict in processed_saliency_maps)
    else:
        assert all(s_map_dict["map_per_image"].shape == (RAW_SIZE, RAW_SIZE) for s_map_dict in processed_saliency_maps)


def _get_pred_result_multiclass(pred_labels):
...
def _get_pred_result_multilabel(pred_labels):
...
def test_process_saliency_maps_in_pred_entity_multiclass() -> None:
    ...
    pred_labels = [torch.tensor(l) for l in PRED_LABELS_TOP_ONE]
    ...
    predict_result = process_saliency_maps_in_pred_entity([predict_result_batch1, predict_result_batch2], explain_config)
...
def test_process_saliency_maps_in_pred_entity_multilabel() -> None:
    ...
    pred_labels = [torch.tensor(l) for l in PRED_LABELS]
