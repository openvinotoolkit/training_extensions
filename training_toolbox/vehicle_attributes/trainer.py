...
from vehicle_attributes.toolbox.utils import get_checkpoint_variable_names
...
def resnet_v1_10_1(features,
  ...
  eval_metric_ops = {
    ...
    "Type accuracy - truck": tf.metrics.accuracy(labels=labels[:, 1], predictions=predictions['types_class'][:, 1]),
    "type accuracy - van": tf.metrics.accuracy(labels=labels[:, 2], predictions=predictions['types_class'][:, 2]),
    "Type accuracy - bus": tf.metrics.accuracy(labels=labels[:, 3], predictions=predictions['types_class'][:, 3])
  }
