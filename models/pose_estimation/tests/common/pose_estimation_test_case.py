...
def create_pose_estimation_test_case(**kwargs):
    expected_outputs_dir = os.path.join(os.path.dirname(__file__), '..', 'expected_outputs')
    TestCase = create_test_case('pose_estimation',
                                **kwargs,
                                metric_keys=['mAP'],
                                expected_outputs_dir=expected_outputs_dir)

    class PoseEstimationTestCase(TestCase):
    ...
    return PoseEstimationTestCase


def create_pose_estimation_export_test_case(**kwargs):
    expected_outputs_dir = os.path.join(os.path.dirname(__file__), '..', 'expected_outputs')
    ExportTestCase = create_export_test_case('pose_estimation',
                                             **kwargs,
                                             metric_keys=['mAP'],
                                             expected_outputs_dir=expected_outputs_dir)

    class PoseEstimationExportTestCase(ExportTestCase):
    ...
    return PoseEstimationExportTestCase
