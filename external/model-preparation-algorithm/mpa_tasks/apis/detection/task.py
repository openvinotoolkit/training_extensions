...
class DetectionInferenceTask(
    ...
    def _init_recipe(self):
        ...
        if self._task_type.domain in {
            ...
            recipe_root = os.path.join(
                MPAConstants.RECIPES_PATH, "stages/instance-segmentation"
            )
        ...
        if train_type == TrainType.SemiSupervised:
            ...
            raise NotImplementedError(
                f"train type {train_type} is not implemented yet."
            )
        ...
        self._patch_datasets(
            self._recipe_cfg, self._task_type.domain
        )  # for OTE compatibility
    ...
    def _init_model_cfg(self):
        ...
        if len(self._anchors) != 0:
            self._update_anchors(
                model_cfg.model.bbox_head.anchor_generator, self._anchors
            )
        return model_cfg
    ...
    def _init_test_data_cfg(self, dataset: DatasetEntity):
        data_cfg = ConfigDict(
            data=ConfigDict(
                train=ConfigDict(ote_dataset=None, labels=self._labels,),
                test=ConfigDict(ote_dataset=dataset, labels=self._labels,),
            )
        )
        return data_cfg
        ...
        for dataset_item, (all_results, feature_vector, saliency_map) in zip(
            ...
            if self._task_type == TaskType.DETECTION:
                shapes = self._det_add_predictions_to_dataset(
                    all_results, width, height, confidence_threshold
                )
    ...
    def _det_add_predictions_to_dataset(
        self, all_results, width, height, confidence_threshold
    ):
        shapes = []
        for label_idx, detections in enumerate(all_results):
            for i in range(detections.shape[0]):
                ...
                assigned_label = [
                    ScoredLabel(self._labels[label_idx], probability=probability)
                ]
    ...
    def _ins_seg_add_predictions_to_dataset(
        self, all_results, width, height, confidence_threshold
    ):
        shapes = []
            ...
            for mask, probability in zip(masks, boxes[:, 4]):
                ...
                for contour, hierarchy in zip(contours, hierarchies[0]):
                    ...
                    if cv2.contourArea(contour) > 0 and polygon.get_area() > 1e-12:
                        shapes.append(
                            Annotation(polygon, labels=labels, id=ID(f"{label_idx:08}"))
                        )
        return shapes
...
class DetectionTrainTask(DetectionInferenceTask, ITrainingTask):
    def save_model(self, output_model: ModelEntity):
        ...
        if hasattr(self._model_cfg.model, "bbox_head") and hasattr(
    ...
    def _generate_training_metrics(
        self, learning_curves, map
    ) -> Optional[List[MetricsGroup]]:
        """
        ...
        for key, curve in learning_curves.items():
            ...
            if n != m:
                logger.warning(
                    f"Learning curve {key} has inconsistent number of coordinates ({n} vs {m}."
                )
                n = min(n, m)
                curve.x = curve.x[:n]
                curve.y = curve.y[:n]
        ...
        output.append(
            BarMetricsGroup(
                ...
                visualization_info=BarChartInfo(
                    "Validation score", visualization_type=VisualizationType.RADIAL_BAR
                ),
            )
        )
