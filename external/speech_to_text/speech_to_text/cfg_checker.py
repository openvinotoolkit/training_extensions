...
def check_quartznet_cfg(cfg):
    ...
    assert "vocab_size" in cfg["tokenizer"], "Vocab size not defined."
    assert isinstance(cfg["tokenizer"]["vocab_size"], int), "Incorrect type of cfg.tokenizer.vocab_size."
    # check audio transforms
    assert "audio_transforms" in cfg, "Audio transforms not defined."
    for dtype in ["train", "val"]:
        assert dtype in cfg["audio_transforms"], f"Audio transforms for {dtype} mode not defined."
    ...
    # check metrics
    assert "metrics" in cfg, "Validation metrics not defined."
    ...
    # check model
    assert "model" in cfg, "Model config not defined."
    assert "dtype" in cfg["model"], "Type of model not defined."
    assert isinstance(cfg["model"]["dtype"], str), "Incorrect type of cfg.model.dtype."
    assert cfg["model"]["dtype"] == "QuartzNet"
    assert "params" in cfg["model"], "Params of model not defined."
    assert "cfg" in cfg["model"]["params"], "Configuration of QuartzNet network not defined."
