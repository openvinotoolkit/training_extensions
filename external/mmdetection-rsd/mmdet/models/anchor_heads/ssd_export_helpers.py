...
def get_proposals(img_metas, cls_scores, bbox_preds, priors, cfg, rescale, cls_out_channels, use_sigmoid_cls,
    ...
    for img_id in range(len(img_metas)):
        cls_score = torch.Tensor(cls_score_list[img_id]).detach().to(priors.device)
        bbox_pred = torch.Tensor(bbox_pred_list[img_id]).detach().to(priors.device)
        img_shape = img_metas[img_id]['img_shape']
        scale_factor = img_metas[img_id]['scale_factor']
        proposals = get_bboxes_single(cls_score, bbox_pred, priors, img_shape, scale_factor, cfg, rescale,
                                      cls_out_channels, use_sigmoid_cls,target_means, target_stds)
        result_list.append(proposals)
    return result_list

def get_bboxes_single(cls_scores, bbox_preds, priors, img_shape, scale_factor, cfg, rescale,
                          cls_out_channels, use_sigmoid_cls, target_means, target_stds):
    ...
    mlvl_bboxes = delta2bbox(priors, bbox_preds, target_means, target_stds, img_shape)
    if rescale:
...
class PriorBox(torch.autograd.Function):
    ...
    @staticmethod
    def symbolic(g, anchor_generator, anchor_stride, feat, img_tensor, target_stds):
        ...
        for ar in anchor_generator.ratios.tolist():
            if ar > 1:
                ars.append(ar)
        return g.op("PriorBox", feat, img_tensor, min_size_f=[min_size], max_size_f=max_sizes,
                    aspect_ratio_f=ars, flip_i=1, clip_i=0, variance_f=list(target_stds), step_f=anchor_stride,
                    offset_f=0.5, step_h_f=0, step_w_f=0, img_size_i=0, img_h_i=0, img_w_i=0)

    @staticmethod
    def forward(ctx, anchor_generator, anchor_stride, feat, img_tensor, target_stds):

        mlvl_anchor = anchor_generator.grid_anchors(feat.size()[-2:], anchor_stride)
...
class PriorBoxClustered(torch.autograd.Function):
    ...
    def symbolic(g, anchor_generator, anchor_stride, feat, img_tensor, target_stds):
        ...
        return g.op("PriorBoxClustered", feat, img_tensor, height_f=heights, width_f=widths,
                    flip_i=0, clip_i=0, variance_f=list(target_stds), step_f=anchor_stride,
                    offset_f=0.5, step_h_f=0, step_w_f=0, img_size_i=0, img_h_i=0, img_w_i=0)
...
class DetectionOutput(torch.autograd.Function):
    ...
    def symbolic(g, cls_scores, bbox_preds, img_metas, cfg,
                   rescale, priors, cls_out_channels, use_sigmoid_cls, target_means, target_stds):

        return g.op("DetectionOutput", bbox_preds, cls_scores, priors,
                    num_classes_i=cls_out_channels, background_label_id_i=0, top_k_i=cfg['max_per_img'],
                    keep_top_k_i=cfg['max_per_img'], confidence_threshold_f=cfg['score_thr'],
                    nms_threshold_f=cfg['nms']['iou_thr'], eta_f=1, share_location_i=1,
    ...
    def forward(ctx, cls_scores, bbox_preds, img_metas, cfg,
        ...
        proposals = get_proposals(img_metas, cls_scores, bbox_preds, priors, cfg, rescale, cls_out_channels,
                                    use_sigmoid_cls, target_means, target_stds)
        batch_size = len(proposals)
        output = torch.zeros(batch_size, 1, cfg.max_per_img, 7).to(cls_scores.device)
        for img_id in range(0, batch_size):
            bboxes, labels = proposals[img_id]
            coords = bboxes[:, :4]
            scores = bboxes[:, 4]
            labels = labels.float()
            output_for_img = torch.zeros(scores.size()[0], 7).to(cls_scores.device)
