...
class Inferencer(ABC):
    ...
    def predict(
        self, image: Union[str, np.ndarray], superimpose: bool = True
...
class TorchInferencer(Inferencer):
    ...
    def post_process(
        self, predictions: Tensor, metadata: Optional[Dict] = None
    ) -> np.ndarray:
        """
    ...
    def __init__(
        ...
        try:
            ...
            model_adapter = OpenvinoAdapter(
                create_core(), model_path=model_file, weights_path=weight_file
            )
            label_names = [label.name for label in self.labels]

            self.configuration = {
                "mean_values": list(
                    np.array(self.config.transform.normalize.mean) * 255
                ),
                "scale_values": list(
                    np.array(self.config.transform.normalize.std) * 255
                ),
                "threshold": threshold,
                "labels": label_names,
            }
            self.model = Model.create_model(
                hparams.inference_parameters.class_name.value,
                model_adapter,
                self.configuration,
            )
            self.model.load()
