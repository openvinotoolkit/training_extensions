...
This section describes the RB-Sparsity (Regularization-Based) algorithm that is implemented in this framework. The method is based on ![L_0](https://latex.codecogs.com/png.latex?%5Cinline%20%5Cdpi%7B120%7D%20L_0) regularization that encourages parameters of the model to become zero:
...
and reparametrize sampling of ![\epsilon_i](https://latex.codecogs.com/png.latex?%5Cinline%20%5Cdpi%7B120%7D%20%5Cepsilon_i)  as:
...
- `params` - parameters of RB-sparsity algorithm:
   ...
   - `schedule` - a type of scheduler that is used to increase the sparsity rate from `sparsity_init` to `sparsity_target`. Can be one of these values:
	   ...
	   In this case the following parameter should be defined in the same scope of parameters:
		   - `sparsity_init` - initial sparsity rate target (![1 - level](https://latex.codecogs.com/png.latex?%5Cdpi%7B120%7D%201%20-%20level) in the objective), e.g. value `0.1` means that at the begging of training, the model will be trained to have 10% of its weights zeroed.
