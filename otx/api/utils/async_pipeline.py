...
class OTXAsyncPipeline(AsyncPipeline):
    """ OTX AsyncPipeline of OTX Detection.

    Args:
        AsyncPipeline (_type_): _description_
    """
    def get_result(self, id):
        """ Get result of inference by id.

        Args:
            id (int): id of inference
        ...
        if result:
            ...
            if "feature_vector" in raw_result or "saliency_map" in raw_result:
                features = (
                    raw_result["feature_vector"].reshape(-1),
                    raw_result["saliency_map"][0],
                )
