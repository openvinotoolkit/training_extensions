...
class Decoder(BlurSegmentation):
    ...
    def resize_and_crop(self, soft_prediction: np.ndarray, original_size: np.ndarray) -> np.ndarray:
        ...
        resized_cropped_soft_prediction = resized_soft_prediction[..., : prepadded_size[0], : prepadded_size[1]]  # type: ignore

        original_size = original_size.astype(np.int64)
        h, w = original_size[0], original_size[1]
        final_soft_prediction = cv2.resize(resized_cropped_soft_prediction, (w, h), 0, 0, interpolation=cv2.INTER_LINEAR)
