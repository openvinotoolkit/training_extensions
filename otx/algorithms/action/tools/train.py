...
def main():
    ...
    if args.gpu_ids is not None or args.gpus is not None:
        ...
        if args.gpu_ids is not None:
            warnings.warn("Non-distributed training can only use 1 gpu now. We will " "use the 1st one in gpu_ids. ")
    ...
    if cfg.checkpoint_config is not None:
        ...
        cfg.checkpoint_config.meta = dict(mmaction_version=__version__ + get_git_hash(digits=7), config=cfg.pretty_text)
