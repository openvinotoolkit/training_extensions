...
class DetectionTrainTask(DetectionInferenceTask, ITrainingTask):
    ...
    def train(
        ...
        results = self._run_task(dataset)
        ...
        val_preds, val_map = self._infer_detector(val_dataset, InferenceParameters(is_evaluation=True), eval=True)
    ...
    def _run_task(self, dataset: DatasetEntity):
        train_dataset = dataset.get_subset(Subset.TRAINING)
        val_dataset = dataset.get_subset(Subset.VALIDATION)
        training_config = prepare_for_training(self._recipe_cfg, train_dataset, val_dataset,
                                               self._time_monitor, self._learning_curves)
