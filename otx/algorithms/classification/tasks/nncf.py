...
from otx.algorithms.classification.adapters.mmcls.nncf import (
    wrap_nncf_model,
)
from otx.algorithms.classification.adapters.mmcls.nncf.utils import (
    build_dataloader,
    get_fake_input,
)
...
class ClassificationNNCFTask(ClassificationInferenceTask, IOptimizationTask):
    ...
    def _initialize_post_hook(self):
        ...
        self._model_cfg.merge_from_dict(initial_model_cfg, False)
        ...
        # Get rid of task
        self._model_cfg.model.pop("task", None)
        # Get rid of TopK
        if (
            self._model_cfg.model.get("multilabel", False)
            or self._model_cfg.model.get("hierarchical", False)
        ):
            self._recipe_cfg.model.head.pop("topk", None)
            self._model_cfg.model.head.pop("topk", None)
        ...
        if getattr(self, "_data_cfg", None) is not None:
            ...
            # drop last batch if last batch has a batch size of 1
            if len(recipe_cfg.data.train.otx_dataset) % recipe_cfg.data.samples_per_gpu == 1:
                recipe_cfg.data.train_dataloader["drop_last"] = True
    ...
    def save_model(self, output_model: ModelEntity):
        ...
        buffer = io.BytesIO()
        hyperparams = self._task_environment.get_hyper_parameters(ClassificationConfig)  # type: ConfigDict
