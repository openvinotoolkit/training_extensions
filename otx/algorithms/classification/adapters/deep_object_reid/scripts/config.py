...
def get_default_config():
    ...
    cfg.model.gcn.hidden_dim_scale = 1.0
    ...
    cfg.loss.softmax = CN()
    cfg.loss.softmax.label_smooth = 0.0  # use label smoothing regularizer
    ...
    cfg.loss.asl = CN()
    cfg.loss.asl.gamma_pos = 0.0
    cfg.loss.asl.gamma_neg = 0.0
    cfg.loss.asl.p_m = 0.05
    cfg.loss.am_binary = CN()
    cfg.loss.am_binary.amb_k = 0.7
    cfg.loss.am_binary.amb_t = 1.0
    ...
    cfg.data.transforms.randaugment.p = 1.0
    ...
    cfg.sc_integration.lr_scale = 1.0
    cfg.sc_integration.epoch_scale = 1.0
    ...
    def _get_list_of_files(cur_path, set_of_files=None):
        ...
        if isinstance(base, list) and len(base) > 1:
            raise NotImplementedError(
                f"Multiple inheritance of configs is not implemented. " f"Please, fix the config {cur_path}"
            )
...
def model_kwargs(cfg, num_classes):
    ...
    return {
        ...
        "similarity_adjustment": cfg.loss.am_binary.amb_t > 1.0,
