...
def build_base_argparser():
    ...
    parser.add_argument("--classes", type=str, nargs="+", help="name of classes in classification dataset")
    parser.add_argument(
        "--custom-roots",
        type=str,
        nargs="+",
    ...
    parser.add_argument("--custom-types", type=str, nargs="+", help="path of custom datasets (delimited by space)")
    parser.add_argument("--custom-names", type=str, nargs="+", help="names of custom datasets (delimited by space)")
...
def build_auxiliary_model(
    ...
    if aux_config_opts:
        print(
            f"applying to aux config changes from command line arguments, "
...
def relu6_symbolic(g, input_blob):
    return g.op("Clip", input_blob, min_f=0.0, max_f=6.0)
...
def patch_InplaceAbn_forward():
    ...
    def forward(self, x):
        ...
        x = nn.functional.batch_norm(
            x, self.running_mean, self.running_var, weight, self.bias, self.training, self.momentum, self.eps
        )
    ...
    if use_gpu:
        ...
        if num_aux_models > 0 and split_models:
            ...
            for model_id in range(num_models):
                device_splits.append([model_id * num_devices_per_model + i for i in range(num_devices_per_model)])
    ...
    if test_only:
        for name, dataloader in datamanager.test_loader.items():
            ...
            if not check_classes_consistency(
                get_model_attr(model, "classification_classes"), dataloader["query"].dataset.classes, strict=False
            ):
                raise ValueError("Inconsistent classes in evaluation dataset")
            if classes and not check_classes_consistency(
                classes_map, get_model_attr(model, "classification_classes"), strict=True
            ):
                raise ValueError("Classes provided via --classes should be the same as in the loaded model")
    elif classes:
        if not check_classes_consistency(classes_map, datamanager.train_loader.dataset.classes, strict=True):
