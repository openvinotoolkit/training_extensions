...
def wrap_nncf_model(model,
    ...
    """
    The function wraps mmcls model by NNCF
    """
    ...
    class MMInitializeDataLoader(PTInitializingDataLoader):
        def get_inputs(self, dataloader_output):
            ...
            # of DataContainer format in mmcls
            kwargs = {
    ...
    def _get_fake_data_for_forward(cfg, get_fake_input_func):
        device = next(model.parameters()).device
        with no_nncf_trace():
            return get_fake_input_func(cfg, device=device)

    def dummy_forward(model):
        fake_data = _get_fake_data_for_forward(cfg, get_fake_input_func)
