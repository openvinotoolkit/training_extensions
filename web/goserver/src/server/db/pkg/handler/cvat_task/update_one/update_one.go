...
var (
	Request = n.RDBCvatTaskUpdateOne
	Queue   = n.QDatabase
)
...
type RequestData = service.CvatTaskUpdateOneRequestData

func encodeRequest(_ context.Context, pub *amqp.Publishing, req interface{}) (err error) {
	b, err := json.Marshal(req.(request))
	if err != nil {
		log.Println("cvat_task.update_one.encodeRequest.Marshal", err)
	}
	pub.Body = b
	return
}
...
func decodeRequest(_ context.Context, deliv *amqp.Delivery) (interface{}, error) {
	...
	if err != nil {
		log.Println("cvat_task.update_one.decodeRequest.Unmarshal", err)
	}
	return req.Data, err
}
...
func decodeResponse(_ context.Context, deliv *amqp.Delivery) (interface{}, error) {
	...
	if err != nil {
		log.Println("cvat_task.update_one.decodeResponse.Unmarshal(deliv.Body, &res)", err)
	}
	b, err := json.Marshal(res.Data)
	if err != nil {
		log.Println("cvat_task.update_one.decodeResponse.Marshal(res.Data)", err)
	}
	err = json.Unmarshal(b, &resData)
	if err != nil {
		log.Println("cvat_task.update_one.decodeResponse.Unmarshal(b, &resData)", err)
	}
	res.Data = resData
	return res, err
}
...
func encodeResponse(_ context.Context, pub *amqp.Publishing, resp interface{}) error {
	...
	if err != nil {
		log.Println("cvat_task.update_one.encodeResponse.Marshal", err)
	}
	pub.Body = b
	return err
}
