...
type RequestData = service.FindInFolderRequestData

func decodeRequest(_ context.Context, deliv *amqp.Delivery) (interface{}, error) {
	var resp request
	err := json.Unmarshal(deliv.Body, &resp)
	if err != nil {
		log.Print("CvatTaskFindInFolder.decodeRequest.Unmarshal", err)
	}
...
func encodeResponse(_ context.Context, pub *amqp.Publishing, req interface{}) error {
	...
	if err != nil {
		log.Print("CvatTaskFindInFolder.encodeResponse.Marshal", err)
	}
	pub.Body = body
	return err
}
